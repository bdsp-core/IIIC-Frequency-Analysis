...
def hilo_dynamic_auto6(
    ...
    eeg_bp, filtwts = iiic_detector.eegfilt(
        seg,
        Fs,
        freq_lo,
        freq_hi,
    ...
    for i in range(eeg_bp.shape[0]):
        this_eeg_bp_smooth = iiic_detector.utils.smooth(
    ...
    for i in range(eeg_bp_smooth.shape[0]):
        ...
        isect1 = iiic_detector.utils.bwselect(
        ...
        isect2 = iiic_detector.utils.bwselect(
    ...
    loc = iiic_detector.utils.locate_events_from_table(
        channels,
        these_events,
        total_samples,
    )
...
def group_events_bi(
    ...
    for channel_idx in range(len(channels)):
        ...
        if this_channel.shape[0] == 0:
            this_channel = iiic_detector.utils.create_nan_df(this_channel)
        ...
        this_result = iiic_detector.calc_periodicity(
            this_channel,
            this_eeg_bp_smooth,
            Fs,
        )
        these_loc = iiic_detector.utils.locate_events_from_table(
    ...
    for i in range(len(these_groups_labels)):
        ...
        this_loc = these_loc.sum(0)
        this_eeg_bp_smooth = eeg_bp_smooth[this_groups_channel_nums, :].sum(0)
        this_eeg_kmeans = iiic_detector.utils.kmeans(
...
def find_events_simple(
    ...
    for i in range(data_log.shape[0]):
        ...
        if not this_result.empty:
            ...
            this_result = iiic_detector.utils.create_nan_df(this_result)
            this_result["channel"] = [channels[i]]
            this_result["channel_num"] = [i]
