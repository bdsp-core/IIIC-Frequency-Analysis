function [ LOC, theseEvents, eeg_bp_smooth ] = fct_findEvents_hilo_dynamic_auto2( seg, Fs,...
    thresh_hi, thresh_lo, smoothWin, freq_lo, freq_hi, boostWin, meanWin, channelLabels)
%fct_findEvents_hilo_dynamic_auto.  Use a high-low threshold approach to find events. Naive to specific BP or montage.
% Accepts thresh_hi, thresh_lo based on percentile.
...
%         theseEvents = cell(size(eeg_bp,1),1);
        ...
        eeg_prctl_hi = simplema(prctile(eeg_bp_smooth, thresh_hi),meanWin,2);
        eeg_prctl_lo = simplema(prctile(eeg_bp_smooth, thresh_lo),meanWin,2);
