function [ LOC, theseEvents, eeg_bp_smooth ] = fct_findEvents_hilo_dynamic_auto6_nleo( seg, Fs,...
    thresh_hi, thresh_lo, smoothWin, freq_lo, freq_hi, boostWin, meanWin, channelLabels)
%fct_findEvents_hilo_dynamic_auto6_nleo.  Use a high-low threshold approach to find events. Naive to specific BP or montage.
% Accepts thresh_hi, thresh_lo based on percentile -- but uses slope == 0
% as the lower theshold. 
...
%            bw_lo1 = diff(thisEEG_bp_smooth)>0;
%            bw_lo2 = diff(thisEEG_bp_smooth)<0;
