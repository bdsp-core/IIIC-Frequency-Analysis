function [ LOC, theseEvents, eeg_bp_smooth ] = fct_findEvents_hilo_dynamic_auto3( seg, Fs,...
    thresh_hi, thresh_lo, smoothWin, freq_lo, freq_hi, boostWin, meanWin, channelLabels)
%fct_findEvents_hilo_dynamic_auto3.  Use a high-low threshold approach to find events. Naive to specific BP or montage.
...
%         theseEvents = cell(size(eeg_bp,1),1);
        ...
        eeg_prctl_hi = prctile(eeg_bp_smooth(:), thresh_hi);
        eeg_prctl_lo = prctile(eeg_bp_smooth(:), thresh_lo);
