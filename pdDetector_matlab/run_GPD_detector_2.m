...
% [LOC, bipolarEvents, eeg_bp_smooth] = fct_findEvents(seg_bi, Fs, thresh_mag, smoothWin, 4, 20, channelLabels_bipolar);
[LOC, bipolarEvents, eeg_bp_smooth] = fct_findEvents_hilo_dynamic(seg_bi, Fs, ...
    thresh_hi, thresh_lo, smoothWin, 4, 8, boostWin,channelLabels_bipolar);
figure(23); showEEG(seg_bi);
figure(24); fct_showEEG_wEvents(seg_bi, bipolarEvents);
fct_vizLOC_bipolar(LOC);
figure(5); showEEG(eeg_bp_smooth/10);
...
%% Using an rms-based auto threshold 
...
[LOC, bipolarEvents, eeg_bp_smooth] = fct_findEvents_hilo_dynamic_auto(seg_bi, Fs, ...
    thresh_hi, thresh_lo, smoothWin, 4, 8, boostWin, meanWin, channelLabels_bipolar);
figure(23); showEEG(seg_bi);
figure(24); fct_showEEG_wEvents(seg_bi, bipolarEvents);
fct_vizLOC_bipolar(LOC);
...
%% Using a moving local percentile-based auto threshold 
...
[LOC, bipolarEvents, eeg_bp_smooth] = fct_findEvents_hilo_dynamic_auto2(seg_bi, Fs, ...
    thresh_hi, thresh_lo, smoothWin, 4,20, boostWin, meanWin, channelLabels_bipolar);
figure(23); showEEG(seg_bi);
figure(24); fct_showEEG_wEvents(seg_bi, bipolarEvents);
fct_vizLOC_bipolar(LOC);
figure(5); showEEG(eeg_bp_smooth/500);
figure(4); fct_showEEG_wEvents(seg_bi, bipolarEvents);
...
%% Using a global percentile-based auto threshold 
...
[LOC, bipolarEvents, eeg_bp_smooth] = fct_findEvents_hilo_dynamic_auto3(seg_bi, Fs, ...
    thresh_hi, thresh_lo, smoothWin, 4,20, boostWin, meanWin, channelLabels_bipolar);
figure(23); showEEG(seg_bi);
figure(24); fct_showEEG_wEvents(seg_bi, bipolarEvents);
fct_vizLOC_bipolar(LOC);
...
n=40;   % 15 = low amplitude, with ?BiPDs?
[seg, seg_bi] = loadFile(files,n);
seg_avg = seg-mean(seg,1);
...
%% Using a purely peaks based method 

seg_bi_1 = seg_bi(1,:);

[pks,locs,w,p]=findpeaks(seg_bi_1,'MinPeakDistance',Fs/10);
figure(31); plot(seg_bi_1); axis tight;hold on;
scatter(locs, pks); 
plot(simplema(seg_bi_1,Fs,2));

hold off;

seg_bi_2 = seg_bi(2,:);

[pks,locs,w,p]=findpeaks(seg_bi_2,'MinPeakDistance',Fs/10);
figure(32); plot(seg_bi_2); axis tight;hold on;
scatter(locs, pks); 
plot(simplema(seg_bi_2,Fs,2));

hold off;

mi

%% Using a per-channel moving auto threshold on NLEO

n=1;   % 15 = low amplitude, with ?BiPDs?
[seg, seg_bi] = loadFile(files,n);
seg_avg = seg-mean(seg,1);

thresh_hi = 90;
thresh_lo = 85;
smoothWin = 24;  % typically 120msec = 24 samples
boostWin = Fs/10;
meanWin = 80;

[LOC, bipolarEvents, eeg_bp_smooth] = fct_findEvents_hilo_dynamic_auto5_nleo(seg_bi, Fs, ...
    thresh_hi, thresh_lo, smoothWin, 4,20, boostWin, meanWin, channelLabels_bipolar);
figure(23); showEEG(seg_bi);
figure(24); fct_showEEG_wEvents(seg_bi, bipolarEvents);
fct_vizLOC_bipolar(LOC);
figure(5); showEEG(eeg_bp_smooth);
figure(7); fct_showEEG_wEvents(seg_bi, bipolarEvents);
